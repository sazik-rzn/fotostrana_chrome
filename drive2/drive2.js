var drive2 = {
    ...
    toolbar: {
        ...
        createHref:function(name, href){
            var link = '<r-yvnv class="c-top-nav"><a href="'+href+'" target="_blank" class="c-top-nav__link" style="cursor:pointer;">' + name + '<span class="c-notification-bubble c-notification-bubble--top" >#</span></a></r-yvnv>';
            $(drive2.toolbar.selector).append(link);
        },
        createLink: function (id, name, onclick) {
            var link = '<r-yvnv class="c-top-nav" id="' + id + '"><a class="c-top-nav__link" style="cursor:pointer;">' + name + '<span class="c-notification-bubble c-notification-bubble--top" >0</span></a></r-yvnv>';
            $(drive2.toolbar.selector).append(link);
            $('#' + id).click(function () {
                onclick();
            });
    ...
    automation: {
        selectors: {
            "Автолайки": '.c-like__button:not(.is-pressed)',
            "Автоподписки": '.c-button--subs:not(.is-pressed)',
        },
        limits:{
			"Автоподписки": 50
		},
        ...
        start: function (selectorName) {
            ...
            drive2.automation.intervals[selectorName] = setInterval(() => {
                if (drive2.automation.buttons[selectorName][drive2.automation.counters[selectorName]]===undefined) {    
                    drive2.automation.buttons[selectorName] = $(drive2.automation.selectors[selectorName]);
                    if(drive2.automation.limits[selectorName]!==undefined && drive2.automation.buttons[selectorName].length>drive2.automation.limits[selectorName]){
                        drive2.automation.buttons[selectorName].length = drive2.automation.limits[selectorName];
                    }
                    drive2.automation.counters[selectorName] = 0;
                }
                $(drive2.automation.buttons[selectorName][drive2.automation.counters[selectorName]]).click();
                 $("#" + drive2.automation.stopButtons[selectorName]+" > a > span").html(drive2.automation.countersShow[selectorName]);
                ...
                drive2.automation.countersShow[selectorName]++;
            }, 500);
        },
        stop: function (selectorName) {
            ...
            drive2.automation.counters[selectorName] = 0;  
            drive2.automation.buttons[selectorName] = [];
        },
        init: function () {
            $.each(drive2.automation.selectors, function (index, selector) {
                ...
                drive2.automation.countersShow[index] = 0;
